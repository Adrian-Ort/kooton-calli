 ...
 * Manages fetching products and inventory, combining them, and setting up filters.
 */

// Global variable to store all combined products once loaded from the API.
let allProducts = [];
...
async function loadAndDisplayProducts() {
    ...
    try {
        ...
        if (!productResponse.ok || !inventoryResponse.ok) {
            ...
            throw new Error('Failed to fetch data from one or more endpoints. Check for CORS errors.');
        }
        ...
        //    Uses correct names from your API response
...
function setupFilters(products) {
    ...
    if (!filterContainer) return; // Exit if filter container not on page
